...
window.ModWebGlRipplesInit = function () {
  ...
  if (typeof window.jQuery === "undefined") {
    console.error("ModWebGlRipplesInit Error: jQuery is required for this plugin but was not found.");
    return;
  }

  // Immediately Invoked Function Expression (IIFE) containing the original plugin code.
  // This executes the entire plugin definition, registering $.fn.ripples.
  (function ($) {
    var gl;
    var $window = $(window);

    // POLYFILL: Adds endsWith for older browsers like early Chrome
    String.prototype.endsWith = function (suffix) {
      return this.indexOf(suffix, this.length - suffix.length) !== -1;
    };

    // Utility function to check for WebGL and required extensions
    ...
    // Utility function to compile GLSL shaders and link a program
    function createProgram(vertexSource, fragmentSource, uniformValues) {
      function compileSource(type, source) {
        ...
        if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
          // Suppressed error: throw new Error('compile error: ' + gl.getShaderInfoLog(shader));
        }
        return shader;
      }
      ...
      if (!gl.getProgramParameter(program.id, gl.LINK_STATUS)) {
        // Suppressed error: console.log(gl.getProgramInfoLog(program.id));
      }

      // Fetch the uniform locations
      program.uniforms = {};
      program.locations = {};
    ...
    // Utility function to activate and bind a texture unit
    ...
    var Ripples = function (el, options) {
      ...
      // Init textures: Load background image
      var image = new Image();
      image.crossOrigin = ""; // Required for CORS/texture security
      ...
      // Init textures for ripple calculations
      this.textures = [];
      this.framebuffers = [];
      ...
      for (var i = 0; i < 2; i++) {
        ...
        if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) != gl.FRAMEBUFFER_COMPLETE) {
          // Suppressed error: throw new Error('Rendering to this texture is not supported (incomplete framebuffer)');
        }
      ...
      // Init GL stuff: setup a quad buffer for drawing
    ...
    Ripples.prototype = {
      ...
      updateTextures: function () {
        ...
        for (var i = 0; i < 2; i++) {
          ...
          gl.uniform1i(this.updateProgram[i].locations.texture, 0); // Need to set texture unit 0 explicitly here
          gl.uniform2fv(this.updateProgram[i].locations.delta, this.textureDelta); // Re-set delta for safety
      ...
      // Complex function to compute UV coordinates based on CSS background properties (cover, contain, position)
      computeTextureBoundaries: function () {
        ...
        // Compute final background width/height based on CSS rules (cover, contain, fixed sizes)
        ...
        // Compute background X and Y position in page coordinates
        ...
        // Calculate normalized UV coordinates for the render shader
      ...
      initShaders: function () {
        ...
        // SHADER 1: Drop Program (Registers the mouse click/move as a ripple on texture 0)
        ...
        // SHADER 2 & 3: Update Programs (Calculates wave propagation and normal vectors)
        this.updateProgram = [0, 0];

        // Update Program [0] (Propagates waves using neighbor average)
        ...
        // Update Program [1] (Calculates normals/offsets based on ripple height)
        this.updateProgram[1] = createProgram(
          ...
          [
            ...
            "info.ba = normalize(cross(dy, dx)).xz;", // Store normal xz in ba channels
            "gl_FragColor = info;",
            "}",
          ].join("\n")
        );
        ...
        // SHADER 4: Render Program (Draws the background image, distorting it with ripple normals)
        this.renderProgram = createProgram(
          ...
          [
            ...
            "varying vec2 backgroundCoord;",
            "void main() {",
            "vec2 offset = -texture2D(samplerRipples, ripplesCoord).ba;", // Fetch normal offset (distortion vector)
      ...
      dropAtMouse: function (e, radius, strength) {
        ...
        // Normalize drop position for the shader
        ...
        // Swap textures for ping-ponging in the next update cycle
    ...
    $.fn.ripples = function (option) {
      return this.each(function () {
        // The original code suppressed this WebGL support check. We maintain the original behavior.
        // if (!supportsWebGL) throw new Error('Your browser does not support WebGL.');
